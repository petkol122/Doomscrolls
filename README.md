...
The TownRoom presence helper also forwards `movementSpeed` when the synced `PlayerPresence` schema carries it. The current connected-world debug player list may show this speed value next to each player together with the existing spawn/position debug text. This is debug presence text only — not gameplay HUD, not prediction, and not client-owned movement.
