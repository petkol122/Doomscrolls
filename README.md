...
The current temporary Notice Board objective is intentionally narrow: interacting with the Notice Board starts a session-temporary objective `Cull Trashboars 0/3`. Completing that objective grants XP once for that session objective only. There is still no quest persistence, quest log, or dialogue system yet.
