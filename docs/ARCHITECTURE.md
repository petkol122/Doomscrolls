...
The current temporary objective layer is also intentionally limited. A Notice Board interaction may start a session-temporary objective `Cull Trashboars 0/3`. Completing that objective grants server-owned XP once per session objective only. There is still no quest persistence, no quest log, and no dialogue/conversation system yet.
