...
# 5a. Add heavy attack constants after ENEMY_ATTACK_WINDUP_MS
...
# 5b. sendEnemyAttackTelegraph signature + payload
...
# 5c. Heavy attack config next to enemy attack config
...
# 5d. Miss branch carries attackKind + heavy cooldown
...
# 5e. Hit branch carries attackKind + heavy damage + heavy cooldown
...
$needle6 = "        } else {`r`n          enemy.nextAttackAtMs = now + enemyAttackCooldownMs;`r`n        }"
$repl6 = "        } else {`r`n          enemy.nextAttackAtMs = now + (resolvedAttackKind === \`"heavy\`" ? heavyAttackCooldownMs : enemyAttackCooldownMs);`r`n        }"
$c = $c.Replace($needle6, $repl6)

$needle7 = "            type: \`"damage_applied\`",`r`n            targetEntityId: landingTarget.characterId as unknown as EntityId,`r`n            sourceEntityId: enemy.id as unknown as EntityId,`r`n            damage: enemyAttackDamage,"
$repl7 = "            type: \`"damage_applied\`",`r`n            targetEntityId: landingTarget.characterId as unknown as EntityId,`r`n            sourceEntityId: enemy.id as unknown as EntityId,`r`n            damage: resolvedDamage,"
$c = $c.Replace($needle7, $repl7)

$needle8 = "            type: \`"enemy_attack_resolved\`",`r`n            enemyId: enemy.id,`r`n            targetEntityId: landingTarget.characterId as unknown as EntityId,`r`n            outcome: \`"hit\`",`r`n            damage: enemyAttackDamage,"
$repl8 = "            type: \`"enemy_attack_resolved\`",`r`n            enemyId: enemy.id,`r`n            targetEntityId: landingTarget.characterId as unknown as EntityId,`r`n            outcome: \`"hit\`",`r`n            attackKind: resolvedAttackKind,`r`n            damage: resolvedDamage,"
$c = $c.Replace($needle8, $repl8)

# 5f. Start-of-telegraph block: choose heavy/normal + pass kind
$needle9 = "      // Start a new telegraph + windup. Damage will only be applied`r`n      // after ENEMY_ATTACK_WINDUP_MS on a future tick.`r`n      enemy.attackLandingAtMs = now + ENEMY_ATTACK_WINDUP_MS;`r`n      enemy.nextAttackAtMs = enemy.attackLandingAtMs + enemyAttackCooldownMs;`r`n`r`n      const telegraphClient = this.clients.find(`r`n        (client) => client.sessionId === targetPlayer.sessionId,`r`n      );`r`n      if (telegraphClient !== undefined) {`r`n        sendEnemyAttackTelegraph(`r`n          telegraphClient,`r`n          enemy.id,`r`n          targetPlayer.characterId,`r`n          ENEMY_ATTACK_WINDUP_MS,`r`n        );"
$repl9 = "      // Start a new telegraph + windup. Damage will only be applied`r`n      // after ENEMY_ATTACK_WINDUP_MS on a future tick.`r`n      // Task 226 -- Brute occasionally rolls a charged heavy attack with a`r`n      // longer windup and heavier damage. All other enemies stay normal.`r`n      const willUseHeavyAttack = enemy.enemyId === \`"trashboar_brute\`"`r`n        && Number.isFinite(heavyAttackChance)`r`n        && heavyAttackChance > 0`r`n        && Math.random() < Math.min(Math.max(heavyAttackChance, 0), 1);`r`n      const chosenAttackKind: \`"normal\`" | \`"heavy\`" = willUseHeavyAttack ? \`"heavy\`" : \`"normal\`";`r`n      const chosenWindupMs = chosenAttackKind === \`"heavy\`" ? heavyAttackWindupMs : ENEMY_ATTACK_WINDUP_MS;`r`n      const chosenCooldownMs = chosenAttackKind === \`"heavy\`" ? heavyAttackCooldownMs : enemyAttackCooldownMs;`r`n      enemy.attackKind = chosenAttackKind;`r`n      enemy.attackLandingAtMs = now + chosenWindupMs;`r`n      enemy.nextAttackAtMs = enemy.attackLandingAtMs + chosenCooldownMs;`r`n`r`n      const telegraphClient = this.clients.find(`r`n        (client) => client.sessionId === targetPlayer.sessionId,`r`n      );`r`n      if (telegraphClient !== undefined) {`r`n        sendEnemyAttackTelegraph(`r`n          telegraphClient,`r`n          enemy.id,`r`n          targetPlayer.characterId,`r`n          chosenWindupMs,`r`n          chosenAttackKind,`r`n        );"
$c = $c.Replace($needle9, $repl9)

Set-Content -Path $f -Value $c -NoNewline
Write-Host "server patches done"
